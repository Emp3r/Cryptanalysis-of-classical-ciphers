...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        let key = "badcfeghijklopnmqrstuvwxyz"
        
        for i in 0...texts.count-2 {
            monoalphabeticTests.append(Monoalphabetic.encrypt(texts[i], with:key))
            
            //print("\(i): \(MonoalphabeticCrack.findMatchesWithUniqueWords(monoalphabeticTests[i]).count)")
            
            //print("  \(MonoalphabeticCrack.uniqueWordsKeyGuess(monoalphabeticTests[i]))");
            
        }
        
        let guess = MonoalphabeticCrack.uniqueWordsKeyGuess(monoalphabeticTests[5])
        
        print("  \(guess)");
        print("  \(Monoalphabetic.decrypt(monoalphabeticTests[5], with: guess))");
